...
class Functions {
  companion object {
    ...
    /** 기본 함수 */
    fun basicFunction(i: Int, j: Int) {
      printFunctionResult(getFunctionName(), i + j)
    }

    /** 리턴 함수 */
    fun basicFunctionReturn(i: Int, j: Int): Int {
      return i + j
    }

    /** 디폴트 파라미터 함수 */
    fun functionWithDefault(i: Int = 10, j: Int = 10) {
      printFunctionResult(getFunctionName(), i + j)
    }

    /** 간략화한 리턴 함수 */
    fun functionSimple(i: Int, j: Int) = i + j

    fun printFunctionResult(callerName: String, result: Any) {
